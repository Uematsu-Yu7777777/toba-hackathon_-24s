...
def translate():
    ...
    if target_lang == "標準語":
        ...
        # 淫夢構文の翻訳を行う
        system_prompt = "次の文章を淫夢構文に変えて。変えた文章のみを出力してほかのものは答えないで。元の文から意味は変えないで。:"
        for translation in inmu_translations:
            if translation["source"] == text:
                translated_text = translation["target"]
                return jsonify({"translated_text": translated_text})
    else:
        system_prompt = f"あなたは文章の口調変換エキスパートです。次の文章を {LANGUAGES[target_lang]} に変換してください。変換後の文章のみを出力し、他の情報は一切含めないでください。:"
